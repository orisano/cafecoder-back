...
//todo reset queue
func initFromDB(toJobQueue *mutexJobQueue, sqlCon **cafecoder.MyCon) {
...
func fromFrontThread(listenfromFront *net.Listener, jobMap *mutexJobMap, toJobQueue *mutexJobQueue) {
	for {
		con, err := (*listenfromFront).Accept()
...
func doFrontThread(con net.Conn, jobMap *mutexJobMap, toJobQueue *mutexJobQueue) {
	...
	if now_worker < cafecoder.MaxWorker {
		//pass the job
...
func fromJudgeThread(listenfromJudge *net.Listener, jobMap *mutexJobMap, toJobQueue *mutexJobQueue, sqlCon **cafecoder.MyCon) {
	for {
...
func doFromJudgeThread(con net.Conn, jobMap *mutexJobMap, toJobQueue *mutexJobQueue, sqlCon **cafecoder.MyCon) {
	//read csv result
...
func passJobToJudge(arg string) {
	conn, err := net.Dial("tcp", cafecoder.JudgeHostPort)
	if err != nil {
		fmt.Println(err)
		return
	}
